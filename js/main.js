...
(function () {
  ...
  /* ---------- 8. 「打个招呼」按钮粒子特效 ----------
     点击在鼠标/触摸位置炸开 16~26 个彩色小圆点，
     随机方向径向爆开、随机大小，快速散开并淡出。
  ...
  function initGreetParticles() {
    ...
    // 温暖 + 品牌色板
    const COLORS = ["#225DAB", "#7BA372", "#CBAF98", "#E8A0BF", "#F2C14E", "#FF7A7A", "#9B8CFF", "#5BC8C0"];
    const rand = function (min, max) { return min + Math.random() * (max - min); };

    function burst(x, y) {
      const n = Math.floor(rand(16, 27)); // 16~26 个
      for (let i = 0; i < n; i++) {
        ...
        const dist = rand(70, 230);          // 径向爆开距离（明显）
        ...
        const anim = p.animate(
          [
            ...
            { transform: "translate(-50%, -50%) translate(" + (dx * 0.7) + "px, " + (dy * 0.7) + "px) scale(0.92)", opacity: 1, offset: 0.3 },
